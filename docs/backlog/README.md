...
| [TICKET-002](tickets/TICKET-002-file-scan.md) | `files` command: recent executables in user-writable folders, Mark-of-the-Web, PE checks, optional YARA | high | 1 | in review (merged; one human check open) |
...
- **Sprint 2: 004, 005, 007.** 007 (GUI) only calls exported functions, so it can go in any
  order; each later command adds its own button. 005 has four independent phases, and each can be its own PR.
- **Sprint 3: 006.**
